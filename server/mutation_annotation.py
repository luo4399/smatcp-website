...
命令行用法：
    ...
    # 处理 TXT 文件（组织作为文件里的 tissue 列指定，无需 --tissue；
    # 该列一格可用逗号/分号/竖线写多个组织，如 "Whole_Blood,Liver"）:
    python mutation_annotation.py --input input_with_tissue.txt --file-type txt -o output.tsv
...
# ──────────────────────────────────────────────────────────────────────────
# CONFIG = {
#     "TFBS": "/media/dubai/home/zhangyun/data/encode/TF_chip/TF_merged.sorted.bed.gz",
#     "TF_motif": "/media/dubai/home/zhangyun/data/encode/motif/TF_motif.sorted.bed.gz",
#     "TF_footprint_dir": "/media/dubai/home/zhangyun/data/encode/footprint/tissue",
#     "cCRE_bed": "/media/dubai/home/zhangyun/data/encode/cCRE/GRCh38-cCREs.bed.gz",
#     "cRE_base": "/media/dubai/home/zhangyun/data/encode/tissue",
#     "tss_pos_bed": os.path.expanduser("~/reference/hg38/annotation/tss_pos.bed"),  # 列: tss_pos, gene_name, strand
#     "gene_link_dir": "/media/dubai/home/zhangyun/data/encode/gene_link",
...
#     "pli_file": "/media/dubai/home/zhangyun/reference/hg38/index/gnomad/gnomad.v2.1.1.lof_metrics.by_gene.txt.bgz",
...
CONFIG = {
    ...
    "tss_pos_bed": "/media/Rome/home/luodl/reference/hg38/annotation/tss_pos.bed",  # 列: tss_pos, gene_name, strand
...
def _output_columns(keep_both_motif_results: bool = False) -> tuple[str, ...]:
    """按选项返回输出列；双结果列紧跟在兼容字段 TF_motif 后。"""
    if not keep_both_motif_results:
        return STANDARD_OUTPUT_COLUMNS
    columns = list(STANDARD_OUTPUT_COLUMNS)
    insert_at = columns.index("TF_motif") + 1
    columns[insert_at:insert_at] = ["TF_motifbreakR", "TF_motif_direct"]
    return tuple(columns)


def _standardize_annotation_output(
    row: pd.DataFrame,
    keep_both_motif_results: bool = False,
) -> pd.DataFrame:
    ...
    if "pLI" in row.columns:
        ...
        row["pLI"] = "."
    output_columns = _output_columns(keep_both_motif_results)
    for column in output_columns:
        if column not in row.columns:
            row[column] = None
    return row.loc[:, output_columns].reset_index(drop=True)


# ──────────────────────────────────────────────────────────────────────────
# 小工具：每个线程持久化复用独立的 pysam.TabixFile 句柄
# ──────────────────────────────────────────────────────────────────────────
_TABIX_THREAD_LOCAL = threading.local()


def _get_tabix_handle(bed_file: str) -> pysam.TabixFile:
    """获取当前线程专属的持久化 TabixFile，避免共享游标和重复打开文件。"""
    handles = getattr(_TABIX_THREAD_LOCAL, "handles", None)
    if handles is None:
        handles = {}
        _TABIX_THREAD_LOCAL.handles = handles
    handle = handles.get(bed_file)
    if handle is None or handle.closed:
        try:
            handle = pysam.TabixFile(bed_file)
        except (OSError, ValueError) as exc:
            raise RuntimeError(f"无法打开Tabix文件: {bed_file}: {exc}") from exc
        handles[bed_file] = handle
    return handle


def _fetch_rows(chrom: str, pos: int, bed_file: str) -> list[list[str]]:
    """
    使用当前线程持久化的 pysam.TabixFile 查询1-based位置 pos。

    pysam.fetch() 接收0-based、half-open坐标，因此查询区间为[pos-1, pos)。
    """
    ...
    if not os.path.exists(bed_file + ".tbi") and not os.path.exists(bed_file + ".csi"):
        ...
        return []
    if pos < 1:
        raise ValueError(f"基因组坐标必须大于等于1: {chrom}:{pos}")

    handle = _get_tabix_handle(bed_file)
    if chrom not in handle.contigs:
        return []
    try:
        return [line.split("\t") for line in handle.fetch(chrom, pos - 1, pos)]
    except (OSError, ValueError) as exc:
        raise RuntimeError(f"Tabix查询失败: {bed_file} {chrom}:{pos}: {exc}") from exc
...
@lru_cache(maxsize=4)
def _load_motifbreakr_results(results_path: str) -> dict[str, str]:
    """读取已有 motifbreakR TSV，并返回 SNP_id -> TF名称串的查找表。

    输入可以是普通文本或 gzip/bgzip 文件。过滤条件与 motifbreakR_query.R 的
    在线查询保持一致：jaspar2022、strong 且 alleleDiff < 0。
    """
    if not os.path.exists(results_path):
        raise FileNotFoundError(f"找不到已有 motifbreakR 结果文件: {results_path}")

    opener = gzip.open if results_path.endswith((".gz", ".bgz")) else open
    names_per_snp: dict[str, list[str]] = defaultdict(list)
    seen_per_snp: dict[str, set[str]] = defaultdict(set)

    with opener(results_path, "rt", encoding="utf-8", errors="replace", newline="") as handle:
        reader = csv.DictReader(handle, delimiter="\t")
        required = {"SNP_id", "geneSymbol", "dataSource", "alleleDiff", "effect"}
        missing = required.difference(reader.fieldnames or [])
        if missing:
            raise ValueError(
                "已有 motifbreakR 结果缺少必要列: " + ", ".join(sorted(missing))
            )

        for row in reader:
            if row["dataSource"].strip() != "jaspar2022":
                continue
            if row["effect"].strip().lower() != "strong":
                continue
            try:
                if float(row["alleleDiff"]) >= 0:
                    continue
            except (TypeError, ValueError):
                continue

            sid = row["SNP_id"].strip()
            name = row["geneSymbol"].strip()
            if not sid or not name or name == "NA" or name in seen_per_snp[sid]:
                continue
            seen_per_snp[sid].add(name)
            names_per_snp[sid].append(name)

    lookup = {sid: ",".join(names) for sid, names in names_per_snp.items()}
    logger.info(
        f"已读取 motifbreakR 结果: {results_path}；"
        f"过滤后 {len(lookup):,} 个变异具有 motif 命中"
    )
    return lookup
...
def annotate_mutation(
    ...
    ref/alt: 仅在 keep_both_motif_results=True、需要计算 motifbreakR 时使用；
        不传时会尝试从 mutation_key（{chrom}_{pos}_{ref}_{alt} 格式）里解析。
    ...
    keep_both_motif_results: 为 True 时分别输出 TF_motifbreakR 和
        TF_motif_direct。TF_motif 始终只使用静态 motif BED 的直接查询结果，
        motifbreakR 不参与 TF_motif 字段及后续评分。
    """
    t_start = time.perf_counter()
    local_times: dict[str, float] = {}
    logger.debug(f"开始处理 mutation_key={mutation_key} tissue={tissue}")
    ...
    with _timer_ctx("TF_motif_annotation", local_times):
        motifbreakr_value = None
        if keep_both_motif_results:
            ...
            if call_ref is not None and call_alt is not None:
                if motif_lookup is not None:
                    ...
                    motifbreakr_value = motif_lookup.get(snp_id)
                    if motifbreakr_value is None:
                        logger.warning(
                            f"批量 motifbreakR 结果中未找到 {snp_id}（不应发生），回退到单条查询"
                        )
                        motifbreakr_value = _tf_motif_via_motifbreakr(chrom, pos, call_ref, call_alt, config)
                else:
                    motifbreakr_value = _tf_motif_via_motifbreakr(chrom, pos, call_ref, call_alt, config)
            else:
                logger.warning(
        ...
        # TF_motif 的默认来源是静态 motif BED；motifbreakR 作为补充来源保留。
        direct_motif_value = _tf_hit_all(config["TF_motif"])
        base["TF_motif"] = (
            direct_motif_value if direct_motif_value is not None else "."
        )
        if keep_both_motif_results:
            base["TF_motifbreakR"] = (
                motifbreakr_value if motifbreakr_value is not None else "."
            )
            base["TF_motif_direct"] = (
                direct_motif_value if direct_motif_value is not None else "."
            )
    ...
    return _standardize_annotation_output(row, keep_both_motif_results)
...
def _keep_best_annotation(annotated: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.debug(f"去重: {n_before:,} 行 -> {len(annotated):,} 行（每个 mutation_key + tissue 保留最高分）")
...
def _build_motif_lookup(
    variants: list[tuple[str, int, str, str]],
    config: dict,
    motifbreakr_results: str | None = None,
) -> dict[str, str] | None:
    """构建 motif 查找表；已有结果文件优先，否则调用 motifbreakR。"""
    if not variants:
        return None
    unique_variants = sorted(set(variants))
    if motifbreakr_results is not None:
        existing = _load_motifbreakr_results(os.path.abspath(motifbreakr_results))
        lookup = {
            f"{chrom}:{pos}:{ref}:{alt}": existing.get(
                f"{chrom}:{pos}:{ref}:{alt}", "."
            )
            for chrom, pos, ref, alt in unique_variants
        }
        n_hit = sum(value != "." for value in lookup.values())
        logger.info(
            f"已有 motifbreakR 结果覆盖本批次全部 {len(lookup):,} 个变异；"
            f"其中 {n_hit:,} 个具有符合条件的 motif 命中，不启动 Rscript"
        )
        return lookup
    logger.debug(f"开始批量调用 motifbreakR，共 {len(unique_variants):,} 个去重后的变异（整批只起一次 R 进程）")
...
def _annotate_task(task: tuple) -> pd.DataFrame:
    """线程池工作函数；task 的固定顺序便于 executor.map 保持输入顺序。"""
    (chrom, pos, mutation_key, tissue, config, ref, alt,
     motif_lookup, keep_both_motif_results) = task
    return annotate_mutation(
        chrom, pos, mutation_key, tissue, config=config,
        ref=ref, alt=alt,
        motif_lookup=motif_lookup,
        keep_both_motif_results=keep_both_motif_results,
    )


def _run_annotation_tasks(tasks: list[tuple], threads: int) -> list[pd.DataFrame]:
    """并行执行注释任务；map 会按输入顺序返回结果，threads=1 时走串行。"""
    if threads < 1:
        raise ValueError("threads 必须大于或等于 1")
    if not tasks:
        return []
    if threads == 1:
        return [_annotate_task(task) for task in tasks]
    logger.debug(f"使用 {threads} 个线程并行注释 {len(tasks):,} 个变异×组织组合")
    with ThreadPoolExecutor(max_workers=threads, thread_name_prefix="mutation") as executor:
        return list(executor.map(_annotate_task, tasks))
...
def annotate_mutation_ids(
    ...
    motif_lookup = (
        _build_motif_lookup(
            [(c, p, r, a) for c, p, r, a, _ in parsed],
            config, motifbreakr_results,
        )
        if keep_both_motif_results else None
    )

    tasks = [
        (chrom, pos, mutation_key, current_tissue, config, ref, alt,
         motif_lookup, keep_both_motif_results)
        for chrom, pos, ref, alt, mutation_key in parsed
        for current_tissue in tissues
    ]
    all_rows = _run_annotation_tasks(tasks, threads)
    annotated = pd.concat(all_rows, ignore_index=True) if all_rows else pd.DataFrame(
        columns=_output_columns(keep_both_motif_results)
    )
    return _keep_best_annotation(annotated) if deduplicate else annotated


def _parse_row_tissues(raw: str, line_no: int) -> list[str]:
    """解析 TXT 文件里 tissue 列的一格：支持逗号/分号/竖线分隔的多个组织名。"""
    parts = [p.strip() for p in re.split(r"[,;|]", raw) if p.strip()]
    if not parts:
        raise ValueError(f"TXT第 {line_no} 行 tissue 列为空")
    invalid = [p for p in parts if p not in TISSUE_ALIAS]
    if invalid:
        raise ValueError(
            f"TXT第 {line_no} 行存在不支持的 tissue: {', '.join(invalid)}；"
...
def annotate_txt(
    ...
    """
    读取制表符分隔的 TXT 文件并批量注释。

    文件必须包含表头，且列为以下两种之一：
        chrom    pos    ref    alt                   （4列，不含组织信息）
        chrom    pos    ref    alt    tissue          （5列，组织由文件本身指定）
    列顺序固定；pos 必须为 1-based 整数坐标。

    组织的确定方式：
      - 若 TXT 文件包含 "tissue" 列：优先使用该行 tissue 列的值（可用逗号/分号/竖线
        分隔多个组织，该行会在每个组织中分别注释）；若某行 tissue 列为空，则回退到
        使用 tissue 参数（若也未提供则报错）。
      - 若 TXT 文件不包含 "tissue" 列：必须通过 tissue 参数指定（可一个或多个），
        每个变异会在每个指定组织中分别注释。
    """
    if not os.path.exists(txt_path):
        raise FileNotFoundError(f"找不到TXT文件: {txt_path}")

    default_tissues: list[str] = (
        [] if tissue is None else ([tissue] if isinstance(tissue, str) else list(tissue))
    )
    invalid_default = [x for x in default_tissues if x not in TISSUE_ALIAS]
    if invalid_default:
        raise ValueError(
            f"不支持的 tissue: {', '.join(invalid_default)}；"
            f"可选值: {', '.join(sorted(TISSUE_ALIAS))}"
        )

    required_columns = ["chrom", "pos", "ref", "alt"]
    columns_with_tissue = required_columns + ["tissue"]
    df = pd.read_csv(txt_path, sep="\t", dtype=str, comment="#")
    has_tissue_column = list(df.columns) == columns_with_tissue
    if not has_tissue_column and list(df.columns) != required_columns:
        raise ValueError(
            "TXT输入格式错误：必须是制表符分隔、包含表头，且列顺序严格为 "
            + "\t".join(required_columns)
            + " 或 "
            + "\t".join(columns_with_tissue)
            + f"；当前列为: {list(df.columns)}"
        )
    if not has_tissue_column and not default_tissues:
        raise ValueError(
            "TXT文件不包含 tissue 列时，必须通过 --tissue 参数指定至少一个组织"
        )

    # ── 先把所有行解析好，收集待注释的变异，供后面一次性批量 motifbreakR 用 ──
    # parsed: (chrom, pos, ref, alt, mutation_key, row_tissues)
    parsed: list[tuple[str, int, str, str, str, list[str]]] = []
    for line_no, rr in enumerate(df.itertuples(index=False), start=2):
        try:
            ...
            mutation_key = f"{chrom}_{pos}_{ref}_{alt}"
            if has_tissue_column:
                raw_tissue = str(getattr(rr, "tissue", "") or "").strip()
                if raw_tissue and raw_tissue.lower() != "nan":
                    row_tissues = _parse_row_tissues(raw_tissue, line_no)
                elif default_tissues:
                    row_tissues = default_tissues
                else:
                    raise ValueError("tissue 列为空，且未通过 --tissue 提供默认组织")
            else:
                row_tissues = default_tissues
            parsed.append((chrom, pos, ref, alt, mutation_key, row_tissues))
        except Exception as exc:
            raise ValueError(f"TXT第 {line_no} 行解析失败: {exc}") from exc

    motif_lookup = (
        _build_motif_lookup(
            [(c, p, r, a) for c, p, r, a, _mk, _ts in parsed],
            config, motifbreakr_results,
        )
        if keep_both_motif_results else None
    )
    ...
    tasks = [
        (chrom, pos, mutation_key, current_tissue, config, ref, alt,
         motif_lookup, keep_both_motif_results)
        for chrom, pos, ref, alt, mutation_key, row_tissues in parsed
        for current_tissue in row_tissues
    ]
    try:
        all_rows = _run_annotation_tasks(tasks, threads)
    except Exception as exc:
        raise ValueError(f"并行注释失败: {exc}") from exc

    annotated = (
        pd.concat(all_rows, ignore_index=True)
        if all_rows else pd.DataFrame(columns=_output_columns(keep_both_motif_results))
    )
    if deduplicate:
        annotated = _keep_best_annotation(annotated)
    if output_path:
        annotated.to_csv(output_path, sep="\t", index=False)
        logger.debug(f"[write] 已写入: {output_path}")
    return annotated
...
def annotate_vcf(
    ...
    t_start = time.perf_counter()
    logger.debug(f"开始读取VCF文件: {vcf_path}")
    n_records = 0
    ...
    with _open_text_auto(vcf_path) as handle:
        for line_no, line in enumerate(handle, start=1):
            ...
            if progress_every and n_records % progress_every == 0:
                ...
                logger.debug(f"...已解析VCF {n_records:,} 条记录，耗时 {elapsed:.1f}s")
    ...
    motif_lookup = (
        _build_motif_lookup(
            [(c, p, r, a) for c, p, r, a, _mk, _ts in pending],
            config, motifbreakr_results,
        )
        if keep_both_motif_results else None
    )
    ...
    tasks = [
        (chrom, pos, mutation_key, current_tissue, config, ref, alt,
         motif_lookup, keep_both_motif_results)
        for chrom, pos, ref, alt, mutation_key, variant_tissues in pending
        for current_tissue in variant_tissues
    ]
    n_variants_attempted = len(tasks)
    if threads < 1:
        raise ValueError("threads 必须大于或等于 1")
    if threads > 1:
        logger.debug(f"使用 {threads} 个线程并行注释 {len(tasks):,} 个变异×组织组合")

    # 按提交顺序收集结果，使并行与串行输出顺序完全一致；单条失败仍沿用原逻辑跳过。
    with ThreadPoolExecutor(max_workers=threads, thread_name_prefix="mutation") as executor:
        futures = [executor.submit(_annotate_task, task) for task in tasks]
        for idx, (task, future) in enumerate(zip(tasks, futures), start=1):
            try:
                all_rows.append(future.result())
            except Exception:
                n_errors += 1
                logger.debug(
                    f"注释失败，跳过该条: mutation_key={task[2]} tissue={task[3]}",
                    exc_info=True,
                )
            if progress_every and idx % progress_every == 0:
                elapsed = time.perf_counter() - t_annotate
                logger.debug(f"...已注释 {idx:,}/{len(tasks):,} 个变异×组织组合，耗时 {elapsed:.1f}s")
...
1) VCF or TXT input via --input & --file-type
   ...
   - For TXT (--file-type txt): fixed columns (chrom, pos, ref, alt), optionally with a
     5th "tissue" column. If the tissue column is present, it takes priority per row
     (comma/semicolon/pipe separated for multiple tissues); --tissue is only needed as
     a fallback for rows with an empty tissue cell, or when the column is absent.
     Multiple tissues may be specified via --tissue, and each variant is annotated in every tissue.

2) Mutation ID input
   Use --mutation-id together with --tissue. IDs may be chr_pos_ref_alt or chr:pos:ref:alt.

Examples:
  %(prog)s --input variants.vcf.gz --file-type vcf --tissue Whole_Blood --threads 8 --output annotation.tsv
  %(prog)s --input variants.txt --file-type txt --tissue Whole_Blood Liver --threads 8 --output annotation.tsv
  %(prog)s --mutation-id chr1_1158636_A_G --tissue Liver --threads 4
  %(prog)s --mutation-id chr10_1370817_G_A --tissue Liver --motifbreakr-results existing.tsv --keep-both-motif-results
""",
    )
    ...
    parser.add_argument(
        "--motifbreakr-results", metavar="TSV",
        help="Reuse an existing motifbreakR TSV/TSV.GZ file. Required columns: SNP_id, "
             "geneSymbol, dataSource, alleleDiff, effect. Matching records are read from "
             "this file and Rscript is not started.",
    )
    parser.add_argument(
        "--keep-both-motif-results", action="store_true",
        help="Run both motifbreakR (or reuse --motifbreakr-results) and the static motif "
             "BED query; add separate TF_motifbreakR and TF_motif_direct output columns. "
             "TF_motif always uses TF_motif_direct; motifbreakR is retained only in its "
             "separate column and does not affect scoring.",
    )
    parser.add_argument(
        "--threads", type=int, default=1, metavar="N",
        help="Number of worker threads for variant-by-tissue annotation (default: 1)",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.threads < 1:
        parser.error("--threads 必须大于或等于 1")
    logger.setLevel(logging.DEBUG if args.verbose else logging.INFO)
    tissues = args.tissue
    if args.motifbreakr_results and not args.keep_both_motif_results:
        parser.error(
            "--motifbreakr-results 仅用于保留独立的 motifbreakR 列；"
            "请同时指定 --keep-both-motif-results"
        )
    ...
    try:
        if args.input:
            ...
            if args.file_type == "txt":
                # 组织可以通过 --tissue 指定，也可以直接作为 TXT 文件里的 tissue 列提供；
                # 若两者都未提供，annotate_txt() 内部会在读取文件后报错。
                result = annotate_txt(
                    args.input,
                    tissue=tissues,
                    output_path=None,
                    ...
                    motifbreakr_results=args.motifbreakr_results,
                    keep_both_motif_results=args.keep_both_motif_results,
                )
    ...
    if args.output:
        ...
        logger.debug(f"结果已写入: {args.output}")
    else:
        result.to_csv(sys.stdout, sep="\t", index=False)
    logger.info(f"总耗时: {time.perf_counter() - total_start:.2f} 秒")
    return 0
